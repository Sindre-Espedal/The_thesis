...
```{r}
library(writexl)
write_xlsx(OD_PM_Ferje_new_test,"potensial_maal\\OD_PM_Ferje_new_test.xlsx")
```

```{r}
P <- Pendling_gk_4k_2022_new_test %>% 
  select(-d_gknr) %>% 
  mutate(Tot_PJ = colSums(.),
         d_gknr = Pendling_gk_4k_2022_new_test$d_gknr) %>% 
  select(d_gknr, Tot_PJ)
```

```{r}
IP_PM <- OD_PM_Ferje_new_test %>% 
  select(o_gknr, d_gknr, dist_ferje, dist_tunell, P_j)
```

```{r}
IP_PM <- IP_PM %>% 
  mutate(ip_potensial_ferje = dist_ferje*(P_j/190934)) %>% 
  mutate(ip_potensial_tunell = dist_tunell*(P_j/190934))
```

```{r}
ikke_parametrisk_potensialmal <- IP_PM %>% 
  group_by(o_gknr) %>% 
  summarise(d_a_ferje = sum(ip_potensial_ferje),
            d_a_tunell = sum(ip_potensial_tunell))
```

```{r}
library(writexl)
write_xlsx(ikke_parametrisk_potensialmal,"potensial_maal\\pm_ip.xlsx")
```

```{r}
ippmtest <- IP_PM %>% 
  group_by(o_gknr) %>% 
  summarise(daf = mean(ip_potensial_ferje), 
            dat = mean(ip_potensial_tunell))
```

```{r}
library(writexl)
write_xlsx(ippmtest,"potensial_maal\\ippm_test.xlsx")
```
